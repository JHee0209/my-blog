...
flex는 [주축(main axis) 하나의 방향으로만 아이템을 정렬한다]({% post_url 2026-09-08-css-flexbox-justify-align %}). 그래서 가로 한 줄, 세로 한 줄을 정렬하는 데는 강하다.

문제는 헤더, 사이드바, 본문, 푸터처럼 가로 방향과 세로 방향을 동시에 맞춰야 하는 전체 페이지 레이아웃이다. flex만 쓰면 컨테이너 안에 컨테이너를 계속 중첩하고, `flex-direction`을 번갈아 바꿔가면서 짜야 한다. 이럴 때 gird는 한 컨테이너 안에서 가로줄과 세로줄을 한 번에 정의할 수 있다.
